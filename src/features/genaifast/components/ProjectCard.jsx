...
function buildPromptText(project, blueprint, step, idx) {
  ...
  return [
    'You are Claude Code helping a learner build "Kundapura Sahayaka" (ಕುಂದಾಪುರ ಸಹಾಯಕ), a Python GenAI assistant for coastal-Karnataka life — temple seva timings and festival dates, local recipes (neer dosa, kori rotti, fish thali), and Kundapura-area travel notes. The stack is plain Python + an LLM API (Anthropic/OpenAI/Groq) + LangChain for RAG + Streamlit for the UI, run locally with a `.env` file for API keys (never hardcode secrets, never commit `.env`). Implement this step directly in the codebase.',
    '',
    ...
    '- Keep the change scoped to this step; do not rewrite unrelated scripts or architecture.',
    '- Keep API keys in `.env` (loaded via `python-dotenv`), never inline in code, and make sure `.env` stays in `.gitignore`.',
    '- Prefer small, readable functions over cleverness — this is a learning project, not a framework.',
    '- Use realistic coastal-Karnataka / Kundapura sample data and questions (seva timings, local dish names, ₹ prices, bus/ferry notes) so the assistant feels grounded in a real place.',
    '',
    'Acceptance checks before finishing:',
    '- Run the script/app and confirm it behaves as described (share the command you used).',
    '- If you touched the vector store or knowledge base, show a sample query and its retrieved chunks/answer.',
    '- If you touched the Streamlit app, confirm `streamlit run app.py` starts cleanly.',
