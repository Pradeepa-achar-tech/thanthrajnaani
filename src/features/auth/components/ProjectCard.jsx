...
function buildPromptText(project, blueprint, step, idx) {
  ...
  return [
    'You are Claude Code working in an ASP.NET Core Web API identity & SSO project — C#, PostgreSQL with EF Core, ASP.NET Core Identity, JWT bearer auth, OpenIddict, Google/Microsoft external logins (the KaavalID app). Implement this step directly in the codebase.',
    '',
    ...
    '- Use clear file names, small focused classes (entities, DTOs, repositories, services, controllers), and beginner-readable code following the existing Clean Architecture layers.',
    '- Add friendly empty, loading, validation, and error states when the step touches a Razor view or API endpoint.',
    '- Use realistic Byndoor, Kundapura, Udupi, or coastal Karnataka sample data (user names, client-app names, tenant/org names) when seeding fixtures.',
    '',
    'Acceptance checks before finishing:',
    '- Run `dotnet build` and `dotnet format` on the touched project; fix all warnings you introduced.',
    '- Run `dotnet test` if tests exist or this step creates tests.',
    '- For database changes, add an EF Core migration (`dotnet ef migrations add ...`) and apply it against PostgreSQL with `dotnet ef database update`.',
    '- If running the app is practical, `dotnet run` and smoke-test the new endpoint/page; report the result.',
