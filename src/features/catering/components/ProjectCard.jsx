...
function buildPromptText(project, blueprint, step, idx) {
  ...
  return [
    ...
    '- Run `flutter analyze` for Flutter changes, `firebase emulators:start` smoke-test for Firestore rules / Functions changes.',
    '- Run the relevant tests if tests exist or this step creates tests.',
    '- If a build/run command is practical, run it and report the result. For Firestore rules or indexes, deploy via `firebase deploy --only firestore:rules,firestore:indexes`.',
