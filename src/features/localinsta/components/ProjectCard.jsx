...
function buildPromptText(project, blueprint, step, idx) {
  ...
  return [
    'You are Claude Code working in a Flutter + Supabase (Postgres) Instagram-style social app project. Implement this step directly in the codebase.',
    '',
    ...
    '- Never hardcode the Supabase URL or anon key in source — read them via --dart-define / a gitignored .env.',
    '- Any new table gets Row Level Security enabled with an explicit policy before it ships — no open tables.',
    '- Use realistic Kundapura, Udupi, or coastal Karnataka usernames and captions (beach photos, temple festivals, local eateries) when seeding fixtures.',
    '',
    'Acceptance checks before finishing:',
    '- Run `dart format .` for touched Dart files; `flutter analyze` for Flutter changes.',
