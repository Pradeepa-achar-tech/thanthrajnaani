...
function buildPromptText(project, blueprint, step, idx) {
  ...
  return [
    'You are Claude Code working in an ASP.NET Core (MVC + Web API) resort management web project — C#, PostgreSQL with Dapper, Google OAuth, QuestPDF, MailKit (the TunMani Resort app). Implement this step directly in the codebase.',
    '',
    ...
    '- Use realistic Byndoor, Kundapura, Udupi, or coastal Karnataka resort sample data (room types, halls, wedding/event bookings, customer names, ₹ pricing) when seeding fixtures.',
    '',
    ...
    '- For database changes, write an idempotent SQL migration (use `IF NOT EXISTS` / `ON CONFLICT`) and apply it against PostgreSQL.',
