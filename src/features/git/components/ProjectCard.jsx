...
function buildPromptText(project, blueprint, step, idx) {
  ...
  return [
    'You are Claude Code helping a learner master Git through hands-on practice on "Tide Board", a small practice repository of Kundapura harbor tide and ferry timings the learner has been growing module by module. The learner is working through a structured Git course and needs you to actually run the git commands for this step in a real local repository (create it with `git init` if it does not exist yet) rather than just describing them. Where the step involves a "teammate" (Asha or Ravi) or a remote, simulate it locally — a second clone folder, or a bare repo made with `git init --bare` — no real GitHub account is required unless the step explicitly says so.',
    '',
    ...
    '',
    'Working rules:',
    '- Run each git command as its own step and show its real output — do not skip straight to the end state.',
    '- Prefer the plain `git` CLI over any GUI so every command is visible and reproducible.',
    '- Keep the change scoped to this step; do not pre-build scenarios that belong to a later module.',
    '- Use realistic Tide Board / Kundapura sample content (ferry times, tide notes, teammate names Asha and Ravi) so the history reads like a real small project, not `foo`/`bar` placeholders.',
    '',
    'Acceptance checks before finishing:',
    '- Run `git log --oneline --graph --all` (or the closest equivalent) and confirm the resulting history actually matches what this step describes.',
    '- Run `git status` and confirm the working tree is in the expected state — clean, unless the step deliberately leaves work in progress.',
    '- If the step involves undoing or rewriting history, show the repository state both before and after so the effect is unmistakable.',
    '- In the final response, list every git command run, in order, and the final repository/branch state.',
  ]
    .filter(Boolean)
    .join('\n')
}
