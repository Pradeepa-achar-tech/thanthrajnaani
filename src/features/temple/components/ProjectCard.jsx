...
function buildPromptText(project, blueprint, step, idx) {
  ...
  return [
    'You are Claude Code working in an Electron + React (Vite) offline desktop app for temple seva management — a local PostgreSQL database (node-postgres / pg), a secure main/preload/renderer split with contextIsolation, and IPC between renderer and main (the "Maranakatte Seva" app for Shri Brahmalingeshwara Temple). No cloud — all data stays on the local machine. Implement this step directly in the codebase.',
    '',
    ...
    '- Respect the Electron security model: keep Node/database access in the main process, expose a minimal typed API over a contextBridge preload, and never enable nodeIntegration in the renderer.',
    '- Use clear file names and small focused modules (main process services, preload bridge, React components/hooks, SQL repositories); write beginner-readable code.',
    '- Add friendly empty, loading, validation, and error states whenever the step touches a React screen or an IPC handler.',
    '- Use realistic Maranakatte / Kundapura coastal-Karnataka temple sample data when seeding fixtures (sevas like Rangapooje, Mangalarathi, Hannikaayi, Yakshagana & Annadhana bookings; devotee names; gotra/nakshatra; ₹ amounts).',
    '',
    'Acceptance checks before finishing:',
    '- Run `npm run lint` (and `npm run build` if the step changes build config); fix all warnings you introduced.',
    '- For database changes, write an idempotent SQL migration (use `CREATE TABLE IF NOT EXISTS` / `ON CONFLICT DO NOTHING`) and apply it against the local PostgreSQL database.',
    '- If running the app is practical, `npm run dev` (or `npm start`) and smoke-test the new screen/IPC channel; report the result.',
