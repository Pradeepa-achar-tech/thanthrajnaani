...
function buildPromptText(project, blueprint, step, idx) {
  ...
  return [
    'You are Claude Code working in a React (Vite) + Supabase (Postgres) artist discovery marketplace project called KalaKaara. Implement this step directly in the codebase.',
    '',
    ...
    '- Use function components and hooks only — no class components, no Tailwind. Plain CSS or CSS Modules.',
    '- Use clear file names, small single-purpose components, and beginner-readable code.',
    '- Add friendly empty, loading, and error states when the step touches user-facing flow.',
    '- Never hardcode the Supabase URL or anon key in source — read them via `import.meta.env.VITE_*` from a gitignored `.env.local`.',
    '- Any new table gets Row Level Security enabled with an explicit policy before it ships — no open tables.',
    '- Never use a paid or card-gated service. If one is the obvious choice, use the free alternative and say why.',
    '- Use realistic Kundapura, Udupi, or coastal Karnataka artist names, art categories, and place names (Maravanthe, Manipal, Brahmavara) when seeding fixtures.',
    '',
    'Acceptance checks before finishing:',
    '- Run `npm run lint` and `npm run build`; both must pass clean.',
    '- For schema changes, write the SQL as a migration file and note the exact `supabase db push` (or SQL editor) command to apply it.',
    '- For RLS changes, state the policy in plain English and confirm it denies cross-user access, not just allows the happy path.',
